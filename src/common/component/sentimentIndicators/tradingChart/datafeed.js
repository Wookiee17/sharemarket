import { fetchIndexData, fetchMomentumSwingData } from './helpers.js';
	...
	getBars: async (symbolInfo, resolution, periodParams, onHistoryCallback, onErrorCallback) => {
		...
		try {
			...
			if(symbolInfo.ticker !== "NSE Index"){
				const resp = await fetchMomentumSwingData({symbolInfo, from: _from, to: _to, time_gap});
				if (resp?.length > 0) {
					resp.forEach(bar => {
				...
				data.forEach(bar => {
					const barTime = moment.utc(bar.created_at).local().unix();
					if (barTime >= from && barTime < to) {
						bars = [...bars, {
							time: barTime * 1000,
							low: bar.low_price,
